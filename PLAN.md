...
- [x] 8.0 UI/UX restyle — two-theme (light "Clarity" indigo-accent / dark "Neutral" grayscale-accent) token-based color system across `apps/web`, user-toggleable via `next-themes`, replacing the ad-hoc hardcoded Tailwind color classes from the earlier dashboard pass — including the fleet/map/digital-twin UI built after this spec was first written. Spec: `docs/superpowers/specs/2026-07-09-ui-ux-restyle-design.md`. Plan: `docs/superpowers/plans/2026-07-10-ui-ux-restyle.md`. **Implemented, reviewed, merged.**
...
- [x] 8.D Maintenance & fleet optimization — the money-saving layer for operators (Delhivery/cab/courier-style fleets). Spec: `docs/superpowers/specs/2026-07-10-maintenance-fleet-optimization-design.md`. Plan: `docs/superpowers/plans/2026-07-10-maintenance-fleet-optimization.md`. **Implemented, reviewed, merged.**
  - **Multi-sensor plugin extensibility**: no new code needed — `Monitorable.telemetry` is already an opaque bag for exactly this reason; documented, not built (no concrete second sensor/hardware to integrate against yet).
  - **Service-due-by-mileage**: `MaintenanceRecord` (`plugins/asset-vehicle`) + `MaintenanceSuggestion` dedup bookkeeping (`services/fleet`) compared against live `odometer_km` telemetry → raises `TaskSuggested` → `services/board` auto-creates a draft task, reusing the existing AI health-score-threshold flow. Suggests once per interval crossing, re-nags only after another full interval passes unaddressed.
  - **Fuel efficiency (km/L)**: computed from `AssetHealthChanged` telemetry deltas in `services/fleet`, honestly skipped (never fabricated) across a refuel or when tank capacity is unknown.
  - **Idle-time detection**: `services/ai-engine` subscribes to `AssetLocationUpdated`, raises `AlertRaised` (info) after a sustained near-zero-speed episode — reuses the existing alert → chat-system-message pipeline, no new consumer needed.
